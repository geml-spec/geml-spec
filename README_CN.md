...
> 状态：`1.0-draft`
> 规范：[English](GEML-spec.md) · [中文](GEML-spec_CN.md)
...
npm install
npm run build
node dist/geml.js ../GEML-spec.geml      # 解析 → 文档模型 JSON
...
GEML 目前处于 **`1.0-draft`**。格式已稳定到足以书写真实文档——本仓库的规范自身就是用 GEML 写的——但在 1.0 正式版之前仍会有所打磨。
...
**Dogfood：** [`GEML-spec.geml`](GEML-spec.geml) 是用 GEML 自身写就的规范，[`GEML-spec.gemlhistory`](GEML-spec.gemlhistory) 是历史格式的样例。一致性由参考解析器的测试集（`npm test`）来检验。

## 仓库结构

```
GEML-spec.md          核心规范（English）
GEML-spec_CN.md        核心规范（中文）
GEML-history-spec.md         .gemlhistory 扩展（English）
GEML-history-spec_CN.md      .gemlhistory 扩展（中文）
GEML-spec.geml         用 GEML 写就的规范（dogfood）
